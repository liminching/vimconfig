...
" syn match	cErrInParen	display contained "[\]{}]\|<%\|%>"
  ...
  syn region	cBlock		transparent fold matchgroup=cBraces start='{' end='}' contains=ALLBUT,@cParenGroup,cCppParen,cCppBracket,cCppString,cBraceError,cErrInBracket
...
syn match cOperator	"<<\|>>\|&&\|||\|++\|--\|->" contained display
syn match cOperator	"[.!~*&%<>^|=,+-]" contained display
syn match cOperator	"/[^/*=]"me=e-1 contained display
syn match cOperator	"/$" contained display
"syn match cOperator	"[][(){};\\]" contained display
syn match cOperatorBold "&&\|\<and\>" contained display
syn match cOperatorBold "||\|\<x\=or\>" contained display
...
" HiLink cUserLabel	Label
  ...
  HiLink cPreCondit	Macro
  HiLink cPreConditIf	Macro
  HiLink cType		Type
